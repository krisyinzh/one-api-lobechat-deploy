...
本指南详细介绍了如何在 Ubuntu 24.04 环境下，通过 Docker 快速构建一套集“多模型中转管理”与“高级智能体对话”于一体的私有化 AI 平台。
...
访问 http://服务器公网IP:3000（默认账号：root，密码：123456）。

在 [渠道] 页面添加你的模型提供商（如 OpenAI, Anthropic）。

在 [令牌] 页面创建一个新令牌，获取以 sk- 开头的密钥。
...
登录 LobeChat：http://服务器IP:8080。

进入 [设置] -> [语言模型] -> [OpenAI]。

API Key：填入 One API 生成的令牌。

接口代理地址 (Proxy URL)：填入 http://服务器私网IP:3000/v1。

点击 [检查连通性]。显示“检查通过”即表示链路完整。
...
有些模型能用，有些不能用：在 One API 的 [日志] 页面查看具体的错误详情。如果是 429 说明模型限流，如果是“无可用渠道”说明模型名称不匹配。
